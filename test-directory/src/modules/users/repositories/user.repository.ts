...
export const addUser = async (data: any): Promise<number | null> => {
  ...
  try {
    ...
    const [confirm] = await pool.query<RowDataPacket[]>(`SELECT EXISTS(SELECT 1 FROM user WHERE email = ?) as isExistEmail;`, [data.email]);
...
export const getUser = async (userId: number): Promise<any | null> => {
  ...
  try {
    const [user] = await pool.query<RowDataPacket[]>(`SELECT * FROM user WHERE id = ?;`, [userId]);
...
export const setPreference = async (userId: number, foodCategoryId: number): Promise<void> => {
  ...
  try {
    await pool.query(`INSERT INTO user_favor_category (food_category_id, user_id) VALUES (?, ?);`, [foodCategoryId, userId]);
  } catch (err) {
    throw new Error(`오류가 발생했어요: ${err}`);
  } finally {
    conn.release();
